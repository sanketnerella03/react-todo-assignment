...
const EditPage = ({match, history}) => {
  ...
  const [task, updateTask] = useState({title: '', dueDate: minDate, status: false});
  useEffect(() => {
    if(match.params.id){
      axios.get(`http://localhost:3000/tasks/${match.params.id}`).then((response) => {
      ...
      <div className='edit-page-container'>
        ...
        <div className="shadow rounded edit-form-section">
          <div className="edit-fields">
            ...
            <input required value={task.title} onChange={handleTitleChange} maxLength="30" className='form-control' type="text" />
          </div>
          <div className="edit-fields">
            <label className='edit-label'>Due Date:</label>
            <input required className='form-control' type="date" min={minDate} value={task.dueDate} onChange={handleDueDateChange} />
          </div>
          <div className="custom-control custom-switch edit-fields">
            <input type="checkbox" checked={task.status} onChange={handleStatusChange} className="custom-control-input" id="customSwitches"></input>
            <label className="custom-control-label" htmlFor="customSwitches">{ task.status ? 'Completed' : 'Not Completed'}</label>
          </div>
